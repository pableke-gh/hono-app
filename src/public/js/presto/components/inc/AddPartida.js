...
export default class AddPartidaInc extends ButtonForm {
	execute() {
		...
		const organica = this.form.elements.orgInc; // organica a incrementar
		...
		api.init().json(url).then(partidaInc => { // fetch partida a incrementar
			...
			this.form.getPartidas().add(partidaInc); // Add and remove PK autocalculated in extraeco.v_presto_partidas_inc
