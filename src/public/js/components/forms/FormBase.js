...
export default class FormBase {
	...
	init() {
		this.#form.$$(this.#opts.refreshSelector).forEach(el => observer.subscribeHtmlElement(el, this.#opts));
	...
	#fnUpdate = (selector, fn) => { // iterate over elements
		this.#form.elements.forEach(selector ? (el => { el.matches(selector) && fn(el); }) : fn);
		return this;
	}
	...
	refresh(model) { observer.emit("form-updated", model); tabs.setHeight(); return this; } // NodeList.prototype.refresh
