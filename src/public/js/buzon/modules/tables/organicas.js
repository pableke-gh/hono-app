...
export default class Organicas extends TableHTML {
	...
	init() {
		...
		this.set("#users", data => usuarios.load(data)); // load users tab

		const fnModal = data => { facturas.setItems("#utProv", data).showModal("dialog#ut"); }
		...
		this.set("#buzon", data => {
			const fnOrganica = uts => (coll.size(uts) > 0)
										...
										: facturas.showError("La orgánica " + data.oCod + " no tiene unidades tramitadoras asociadas.");
		...
		tabs.setAction("report", () => {
			fnReport(facturas.getValue("utProv"), buzon.getIdOrganica());
			facturas.closeModal(); // always close modal
		});
	}
