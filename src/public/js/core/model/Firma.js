...
class Firma {
	...
	myFlag = data => {
		...
		if (this.#fnAceptada(data.fmask)) // bandera verde de aceptada (fmask = mascara de firma)
			return '<i class="fas fa-flag text-green icon" title="Solicitud firmada"></i>';
		if (this.#fnRechazada(data.fmask, data.info)) // bandera roja de rechazada (fmask = mascara de firma)
			return '<i class="fas fa-flag text-error icon" title="Solicitud rechazada"></i>';
		return '<i class="fas fa-flag text-warn icon" title="Solicitud pendiente de firma"></i>'
	}
	render = data => {
		const fecha = i18n.isoDate(data.fecha); // plantilla para el listado de firmas principales
		const aceptada = `<i class="fas fa-flag text-green icon" title="Firma aceptada"></i>Aceptada ${fecha}`;
		const rechazada = `<i class="fas fa-flag text-error icon" title="${data.info}"></i>Rechazada ${fecha}`;
		const pendiente = '<i class="fas fa-flag text-warn icon" title="Pendiente de firma"></i>Pendiente';
